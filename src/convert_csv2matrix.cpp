...
Eigen::MatrixXd load_csv64(const std::string & path) {
    ...
    // Eigen::MatrixXd q_traj_csv = matrix.block(0,0,7,columns[0].size());
    Eigen::MatrixXd q_traj_csv = matrix.block(0,0,64,columns[0].size());
...
Eigen::MatrixXd load_csv_8(const std::string & path) {
    ...
    Eigen::MatrixXd q_traj_csv = matrix.block(0,0,8,columns[0].size()); 
    // std::cout<<"cq_traj_csv:"<<q_traj_csv.col(0)<<std::endl; 
    return q_traj_csv;
}
