                        ...
                        const Matrix<double,6,1> &dx_last)
{
    ...
    MatrixXd J_goal_ana;
    MatrixXd J_goal_geo;
    J_goal_ana = realsenserobot.pose_jacobian(q_desired);
    J_goal_geo = geomJac(realsenserobot, J_goal_ana, q_desired, 8);

    J_geom_goal = J_goal_geo.block(0,3,3,8);
    std::cout << "J_geom_goal: " <<J_geom_goal<< std::endl;
    abort();
    ...
    //     Matrix<c_float, 7, 7> H =   Jm_t.transpose()*Jm_t +  J.transpose() *W_cart *J;
    //     H_s = H.sparseView();
